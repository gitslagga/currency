...
class ChartController extends HomeController
{
	...
	{
		if ($market) {
			$data = (QQ3479015851_DEBUG ? null : S('ChartgetJsonData' . $market));
	...
	public function getMarketTrendJson()
	{
		...
		$input = I('get.');
		$market = (is_array(C('market')[$input['market']]) ? trim($input['market']) : C('market_mr'));
		$data = (QQ3479015851_DEBUG ? null : S('ChartgetMarketTrendJson' . $market));
	...
	public function getMarketOrdinaryJson()
	{
		...
		$timeaa = (QQ3479015851_DEBUG ? null : S('ChartgetMarketOrdinaryJsontime' . $market . $time));
		...
		$tradeJson = (QQ3479015851_DEBUG ? null : S('ChartgetMarketOrdinaryJson' . $market . $time));
	...
	public function getMarketSpecialtyJson()
	{
		...
		$timeaa = (QQ3479015851_DEBUG ? null : S('ChartgetMarketSpecialtyJsontime' . $market . $time));
		...
		$tradeJson = (QQ3479015851_DEBUG ? null : S('ChartgetMarketSpecialtyJson' . $market . $time));
