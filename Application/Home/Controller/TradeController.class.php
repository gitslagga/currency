...
class TradeController extends HomeController
{
	...
	public function chartqq3479015851($qq3479015851 = NULL){
		
		if (!$qq3479015851) {
			$qq3479015851 = C('market_mr');
		}
		...
		$this->assign('qq3479015851', $qq3479015851);
		$this->assign('qq3479015851_jiaoyiqu', $qq3479015851_getCoreConfig['qq3479015851_indexcat']);
		$this->assign('qq3479015851_marketjiaoyiqu', $jiaoyiqu);
		$this->assign('qq3479015851_xnb', explode('_', $qq3479015851)[0]);
		$this->assign('qq3479015851_rmb', explode('_', $qq3479015851)[1]);
		$this->display();
	...
	public function upTrade($paypassword = NULL, $market = NULL, $price, $num, $type)
	{
		...
		if(!$hou_price){
			$hou_price = C('market')[$market]['qq3479015851_faxingjia'];
		}
	...
	public function matchingTrade($market = NULL)
	{
		...
		for (; true; ) {
			$buy = $mo->table('qq3479015851_trade')->where(array('market' => $market, 'type' => 1,'userid' => array('gt',0), 'status' => 0))->order('price desc,id asc')->find();
			$sell = $mo->table('qq3479015851_trade')->where(array('market' => $market, 'type' => 2,'userid' => array('gt',0),'status' => 0))->order('price asc,id asc')->find();
			...
			if ($buy && $sell && (0 <= floatval($buy['price']) - floatval($sell['price']))) {
				...
				if (check_arr($rs)) {
					...
					S('qq3479015851_allcoin'.$jiaoyiqu,null);
	...
	public function matchingAutoTrade($market = NULL)
	{
		...
		mlog("duqunew_trade_".$new_trade_qq3479015851);
		
		if ($new_trade_qq3479015851) {
			
			mlog("wojinlail".$new_trade_qq3479015851);
