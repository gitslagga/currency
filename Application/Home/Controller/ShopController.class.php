...
class ShopController extends HomeController
{
	public function index($name = NULL, $type = NULL, $deal = NULL, $addtime = NULL, $price = NULL, $ls = 20)
	{
		...
		$showText = QQ3479015851_DEBUG ? null : S('ShopText');
		
		if($showText === ""){
			$showText = D('Text')->get_content('game_shop');
			S('ShopText',$showText);
		}
		...
		$shop_type_list = QQ3479015851_DEBUG ? null : S('shop_type_list');
		
		if($shop_type_list === ""){
			$shop_type_list = D('Shop')->shop_type_list();
			S('shop_type_list',$shop_type_list);
		}
	...
	public function view($id)
	{
		...
		$showText = QQ3479015851_DEBUG ? null : S('game_shop_view');
		
		if($showText === ""){
			$showText = D('Text')->get_content('game_shop_view');
			S('game_shop_view',$showText);
		}
	...
	public function log($ls = 15)
	{
		...
		$showText = QQ3479015851_DEBUG ? null : S('game_shop_log');
		
		if($showText === ""){
			$showText = D('Text')->get_content('game_shop_log');
			S('game_shop_log',$showText);
		}
		$this->assign('prompt_text',$showText);
