...
class VoteController extends HomeController
{
	public function index()
	{
		...
		$coin_list = QQ3479015851_DEBUG ? null : S('VoteType');
		...
		if (is_array($coin_list)) {
			foreach ($coin_list as $k => $v) {
				...
				$zhichi = QQ3479015851_DEBUG ? null : S('Votezhichi'.$vv['coinname']);
				...
				$fandui = QQ3479015851_DEBUG ? null : S('Votefandui'.$vv['coinname']);
				if($fandui===false){
		...
		$showText = QQ3479015851_DEBUG ? null : S('VoteText');
