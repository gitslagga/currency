...
class AjaxController extends HomeController
{
	...
	{
		$data = (QQ3479015851_DEBUG ? null : S('getJsonMenu'));
	...
	public function top_coin_menu($ajax = 'json')
	{
		...
		$data = (QQ3479015851_DEBUG ? null : S('qq3479015851_getTopCoinMenu'));
...
/* 				if (C('market')[$v['name'] . '_cny']['new_price']) {
	...
	{
		$data = (QQ3479015851_DEBUG ? null : S('allsum'));
	...
	{
		$data = (QQ3479015851_DEBUG ? null : S('allcoin'));
	...
	public function allcoin_a($id=0,$ajax = 'json')
	{
		
		$jiaoyiqu = (QQ3479015851_DEBUG ? null : S('jiaoyiqu'));
		...
		$data = (QQ3479015851_DEBUG ? null : S('qq3479015851_allcoin'.$id));
        ...
        // 市场交易记录
		$themarketLogs = (QQ3479015851_DEBUG ? null : S('marketjiaoyie24'));
		...
		if (!$data) {
			...
			foreach ($jiaoyiqu[$id] as $k => $v) {
				...
				$qq3479015851_data['url'][$v][0] = C('market')[$v]['title'];
				$qq3479015851_data['url'][$v][1] = round(C('market')[$v]['new_price'], C('market')[$v]['round']);
				$qq3479015851_data['url'][$v][2] = C('market')[$v]['buy_price'] ? round(C('market')[$v]['buy_price'], C('market')[$v]['round']):0;
				$qq3479015851_data['url'][$v][3] = C('market')[$v]['sell_price']? round(C('market')[$v]['sell_price'],C('market')[$v]['round']):0;
				$qq3479015851_data['url'][$v][4] = isset($themarketLogs[$v]) ? $themarketLogs[$v] : 0;
				$qq3479015851_data['url'][$v][5] = '';
				$qq3479015851_data['url'][$v][6] = round(C('market')[$v]['volume'], 2) * 1;
				$qq3479015851_data['url'][$v][7] = round(C('market')[$v]['change'], 2);
				$qq3479015851_data['url'][$v][8] = C('market')[$v]['name'];
				$qq3479015851_data['url'][$v][9] = C('market')[$v]['xnbimg'];
				$qq3479015851_data['url'][$v][10] = '';
			}
			//var_dump($qq3479015851_data);
			$data = $qq3479015851_data;
			S('qq3479015851_allcoin'.$id, $qq3479015851_data);
			
			//var_dump(  S('qq3479015851_allcoin'.$id) );
		}
	...
	public function allcoin_a_test($id=0,$ajax = 'json')
	{
		...
		//var_dump(C('market')['com_cny']['buy_price']);
	...
	{
		$data = (QQ3479015851_DEBUG ? null : S('trends'));
	...
	{
		$data = (QQ3479015851_DEBUG ? null : S('getJsonTop' . $market));
		...
		if (!$data) {
			if ($market) {
				...
				if(C('market')[$market]['zhang']>0){
					if(C('market')[$market]['hou_price']>0){
						$data['info']['zhang'] = C('market')[$market]['hou_price']+floatval(C('market')[$market]['hou_price']*floatval((C('market')[$market]['zhang'])/100));
					}
				}else{
					$data['info']['zhang'] = '';
				}
				
				if(C('market')[$market]['die']>0){
					if(C('market')[$market]['hou_price']>0){
						$data['info']['die'] = C('market')[$market]['hou_price']-floatval(C('market')[$market]['hou_price']*floatval((C('market')[$market]['die'])/100));
					}
				}else{
					$data['info']['die'] = '';
				}
	...
	{
		$data = (QQ3479015851_DEBUG ? null : S('getTradelog' . $market));
		if (!$data) {
	...
	{
		$data = (QQ3479015851_DEBUG ? null : S('getAwardInfo'));
		if (!$data) {
	...
	public function getDepth($market = NULL, $trade_moshi = 1, $ajax = 'json')
	{
		...
		$data_getDepth = (QQ3479015851_DEBUG ? null : S('getDepth'));
	...
	{
		$chat = (QQ3479015851_DEBUG ? null : S('getChat'));
